...
# KMS Keys (RULE 17, RULE 22, RULE 30, RULE 45, RULE 49)
...
# Database Subnets (RULE 1, RULE 2, RULE 12)
...
resource "aws_security_group" "db" {
  ...
  # Ingress from App SG (PostgreSQL 5432)
  ingress {
...
resource "aws_security_group" "elasticache" {
  ...
  # Ingress from App SG (Redis 6379)
  ingress {
...
resource "aws_security_group" "vpc_endpoints" {
  ...
  # Ingress from App SG for various services
  ingress {
    description     = "HTTPS from App tier to VPC Endpoints"
    protocol        = "tcp"
    from_port       = 443
    to_port         = 443
    security_groups = [aws_security_group.app.id]
  ...
  ingress {
    description     = "SSM from App tier to VPC Endpoints"
    protocol        = "tcp"
    from_port       = 443
    to_port         = 443
    security_groups = [aws_security_group.app.id]
  ...
  ingress {
    description     = "ECR Docker from App tier to VPC Endpoints"
    protocol        = "tcp"
    from_port       = 443
    to_port         = 443
    security_groups = [aws_security_group.app.id]
...
# DB Subnet Group (RULE 26, RULE 55)
...
resource "aws_iam_role" "flow_logs" {
  name_prefix = "three-tier-web-app-pro-flow-logs-role-" # RULE 26, RULE 58
...
# CloudTrail (RULE 38, RULE 20, RULE 59)
...
resource "aws_iam_role" "cloudtrail" {
  name_prefix = "three-tier-web-app-pr-cloudtrail-role-" # RULE 26, RULE 58
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Action    = "sts:AssumeRole"
...
resource "aws_iam_role" "config" {
  name_prefix = "three-tier-web-app-produc-config-role-" # RULE 26, RULE 58
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Action    = "sts:AssumeRole"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        Action   = ["logs:CreateLogStream", "logs:PutLogEvents"]
        Effect   = "Allow"
        Resource = "arn:aws:logs:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:log-group:/aws/config/*"
      }
    ]
  })
}
...
# CloudWatch Security Metric Filters and Alarms (RULE 35, RULE 45)
...
resource "aws_vpc_endpoint" "ec2" {
  ...
  security_group_ids  = [aws_security_group.vpc_endpoints.id]
  private_dns_enabled = true
  tags                = merge(var.tags, { Name = "${var.project_name}-${var.environment}-ec2-vpce" })
}
...
resource "aws_vpc_endpoint" "ec2_messages" {
  ...
  security_group_ids  = [aws_security_group.vpc_endpoints.id]
  private_dns_enabled = true
  tags                = merge(var.tags, { Name = "${var.project_name}-${var.environment}-ec2-messages-vpce" })
}
