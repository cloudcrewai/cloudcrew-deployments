...
resource "aws_subnet" "public" {
  ...
  tags                    = merge(var.tags, { Name = "${var.project_name}-public-${count.index + 1}", Tier = "public" })
}
...
resource "aws_subnet" "private" {
  ...
  availability_zone       = local.az_list[count.index]
  map_public_ip_on_launch = false # RULE 12
  tags                    = merge(var.tags, { Name = "${var.project_name}-private-${count.index + 1}", Tier = "private" })
}

resource "aws_eip" "nat" {
  count  = var.az_count
  domain = "vpc"
  tags   = merge(var.tags, { Name = "${var.project_name}-eip-${count.index + 1}" })
}
...
resource "aws_route_table" "private" {
  count  = var.az_count
  vpc_id = aws_vpc.main.id
  tags   = merge(var.tags, { Name = "${var.project_name}-rt-private-${count.index + 1}" })
}
...
resource "aws_cloudwatch_log_group" "flow_logs" {
  ...
  retention_in_days = var.log_retention_days # RULE 40
  kms_key_id        = aws_kms_key.logs.arn   # Encrypt CloudWatch log groups with KMS CMK (Shield Remediation Map)
  tags              = var.tags
}

resource "aws_iam_role" "flow_logs" {
  name_prefix = "hipaa-data-governance-flow-logs-role-" # RULE 58: Short role name
...
resource "aws_iam_role_policy" "flow_logs" {
  name = "${var.project_name}-${var.environment}-flow-logs-policy" # RULE 58: Short policy name
...
# S3 VPC Gateway Endpoint (RULE 43)
