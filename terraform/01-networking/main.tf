...
resource "aws_db_subnet_group" "main" {
  name_prefix = "${var.project_name}-${var.environment}-db-subnet-group-" # RULE 58
...
resource "aws_security_group" "vpc_endpoints" {
  ...
  # Ingress from VPC CIDR for all ports
  ingress {
  ...
  # Egress to VPC CIDR only (RULE 8)
  egress {
...
resource "aws_security_group" "default" {
  ...
  # Deny all ingress and egress (RULE 8b, Shield Remediation Map)
  ingress {
...
# VPC Endpoints (RULE 43, "VPC endpoint exported attributes" rule, "VPC endpoints for AWS services" rule)
# Interface Endpoints
...
# KMS Key for general purpose (e.g. SNS, Secrets Manager)
...
resource "aws_cloudwatch_log_group" "flow_logs" {
  ...
  kms_key_id        = aws_kms_key.logs.arn # Encrypt with logs KMS key
  tags              = var.tags
}

resource "aws_iam_role" "flow_logs" {
  name_prefix = "aurora-postgres-prod-p-flow-logs-role-" # RULE 26, RULE 58
...
resource "aws_s3_bucket" "cloudtrail_logs" {
  bucket_prefix = "aurora-postgres-prod-cloudtrail-logs-" # RULE 58
  force_destroy = false # RULE 16, RULE 38
  tags          = var.tags
...
resource "aws_s3_bucket_server_side_encryption_configuration" "cloudtrail_logs" {
  ...
  rule {
    apply_server_side_encryption_by_default {
      ...
      kms_master_key_id = aws_kms_key.s3.arn # Encrypt with S3 KMS key
    }
...
resource "aws_cloudwatch_log_group" "cloudtrail" {
  ...
  retention_in_days = var.audit_log_retention_days # RULE 38: audit-relevant group
  kms_key_id        = aws_kms_key.logs.arn # Encrypt with logs KMS key
  tags              = var.tags
}

resource "aws_iam_role" "cloudtrail" {
  name_prefix = "aurora-postgres-prod-cloudtrail-role-" # RULE 26, RULE 58
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
...
resource "aws_s3_bucket" "config_bucket" {
  bucket_prefix = "aurora-postgres-prod-p-config-bucket-" # RULE 58
  force_destroy = false
  tags          = var.tags
...
resource "aws_iam_role" "config" {
  name_prefix = "aurora-postgres-prod-prod-config-role-" # RULE 26, RULE 58
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Action    = "sts:AssumeRole"
