...
# KMS Keys and Aliases (RULE 17, RULE 22, RULE 29, RULE 45, RULE 49, RULE 30, RULE 32, RULE 33, RULE 34)
...
# SNS Topics and Policies (RULE 45)
...
# S3 Buckets and related configurations (RULE 30)
resource "aws_s3_bucket" "cloudtrail_logs" {
  bucket_prefix = "iot-data-pipeline-pr-cloudtrail-logs-" # RULE 58
...
resource "aws_s3_bucket" "config_logs" {
  bucket_prefix = "iot-data-pipeline-produc-config-logs-" # RULE 58
  force_destroy = false                                                # RULE 30
  tags          = var.tags
...
# IAM Roles and Policies (RULE 13, RULE 38, RULE 34)
resource "aws_iam_role" "flow_logs" {
  name_prefix = "iot-data-pipeline-prod-flow-logs-role-" # RULE 58
...
resource "aws_iam_role" "cloudtrail" {
  name_prefix = "iot-data-pipeline-pro-cloudtrail-role-" # RULE 58
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
...
resource "aws_iam_role" "config" {
  name_prefix = "iot-data-pipeline-product-config-role-" # RULE 58
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
...
resource "aws_vpc" "main" {
  ...
  enable_dns_support   = true  # RULE "VPC DNS settings"
  enable_dns_hostnames = true  # RULE "VPC DNS settings"
...
resource "aws_route_table" "database" {
  ...
  # No default route to IGW or NAT Gateway for database subnets
}
...
# Security Groups (RULE 8, RULE 51)
resource "aws_security_group" "vpc_egress" {
  name        = "${var.project_name}-${var.environment}-vpc-egress-sg"
  description = "Allows all outbound traffic within the VPC CIDR"
  vpc_id      = aws_vpc.main.id
  tags        = var.tags
...
resource "aws_security_group" "config" {
  name        = "${var.project_name}-${var.environment}-config-sg"
  description = "Security group for AWS Config delivery"
  vpc_id      = aws_vpc.main.id
  tags        = var.tags
...
# VPC Endpoints (RULE "VPC endpoints for AWS services", RULE 43)
...
# CloudWatch Security Metric Filters and Alarms (RULE 34, RULE 35)
