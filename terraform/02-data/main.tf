...
resource "aws_kms_key" "s3" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      {
        Sid    = "AllowS3AndCloudTrailUseOfKey"
        Effect = "Allow"
        Principal = {
          Service = [
            "s3.amazonaws.com",
...
resource "aws_iam_role" "cloudtrail" {
  name_prefix = "ecs-microservices-pro-cloudtrail-role-" # Fixed hardcoded prefix
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Action    = "sts:AssumeRole"
...
resource "aws_iam_role" "flow_logs" {
  name_prefix = "${var.project_name}-${var.environment}-flow-logs-role-"
  assume_role_policy = jsonencode({
    Version = "2012-10-17", # Added comma
    Statement = [{ Action = "sts:AssumeRole", Effect = "Allow", Principal = { Service = "vpc-flow-logs.amazonaws.com" } }] # Added commas
  })
  tags = var.tags
  lifecycle {
    create_before_destroy = true
...
resource "aws_s3_bucket" "cloudtrail_logs" {
  bucket_prefix = "ecs-microservices-pr-cloudtrail-logs-" # Fixed hardcoded prefix
  force_destroy = false # Production setting
  tags          = var.tags
...
resource "aws_s3_bucket" "config_logs" {
  bucket_prefix = "${var.project_name}-${var.environment}-config-logs-"
  force_destroy = false # Production setting
  tags          = var.tags
...
resource "aws_security_group_rule" "vpc_endpoints_ingress_self" {
  type                     = "ingress"
  from_port                = 0
  to_port                  = 0
  protocol                 = "-1"
...
resource "aws_security_group_rule" "vpc_endpoints_ingress_private_subnets" {
  type              = "ingress"
  from_port         = 0
  to_port           = 0
  protocol          = "-1"
...
resource "aws_rds_cluster_parameter_group" "main" {
  ...
  parameter {
    name  = "log_statement"
    value = "all" # Required for query logging
  }
...
resource "aws_db_subnet_group" "main" {
  name_prefix = "${var.project_name}-${var.environment}-aurora-subnet-group-"
  subnet_ids  = data.terraform_remote_state.networking.outputs.private_subnet_ids
  description = "Aurora DB subnet group for ${var.project_name} ${var.environment}"
  tags        = var.tags
...
resource "aws_rds_cluster" "main" {
  ...
  backup_retention_period     = 30 # Production setting
...
resource "aws_rds_cluster_instance" "standby" {
  identifier          = "${var.project_name}-${var.environment}-aurora-standby"
  cluster_identifier  = aws_rds_cluster.main.id
  instance_class      = var.db_instance_class
  engine              = aws_rds_cluster.main.engine
  engine_version      = aws_rds_cluster.main.engine_version
...
resource "aws_elasticache_replication_group" "main" {
  ...
  num_cache_clusters            = var.az_count # One primary, one replica per AZ
  port                          = 6379
...
resource "aws_cloudtrail" "main" {
  ...
  kms_key_id                    = aws_kms_key.s3.arn # CloudTrail uses S3 KMS key for its own logs
  tags                          = var.tags
...
resource "aws_sns_topic" "config" {
  ...
  kms_master_key_id = aws_kms_key.logs.arn # Using logs KMS key for SNS
  tags              = var.tags
}
...
resource "aws_config_config_rule" "cloud_trail_encryption_enabled" {
  ...
  description = "Checks whether CloudTrail logs are encrypted with KMS."
  source {
    owner             = "AWS"
...
resource "aws_config_config_rule" "guardduty_enabled_centralized" {
  name        = "${var.project_name}-${var.environment}-guardduty-enabled-centralized"
...
resource "aws_s3_bucket" "cloudtrail_logs_access" {
  ...
  force_destroy = false # Changed to false for production
}
